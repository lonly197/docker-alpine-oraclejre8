...
RUN	set -x \
    ...
    && cd /tmp/ \
    && wget --no-check-certificate -c -q --header "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/${VERSION}-b${JAVA_BUILD}/${JAVA_PATH}/jre-${VERSION}-linux-x64.tar.gz \
